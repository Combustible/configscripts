...
RUN groupadd --non-unique -g $DOCKER_GID docker && \
	apt-get update && \
	apt-get install -y build-essential git zsh openssh-client less man tmux tree ncdu pv python-dev cmake ctags g++ wget gdb cscope openssh-server astyle libncurses5-dev libatk1.0-dev docker.io && \
	mkdir /var/run/sshd

ENV USERHOME /home/$USERNAME

RUN groupadd --non-unique -g $GID $USERNAME && \
	useradd -mNs /usr/bin/zsh -u $UID -g $GID $USERNAME && \
